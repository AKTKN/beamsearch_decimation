...
SOURCES = {
    "ldpc": ("https://github.com/quantumgizmos/ldpc", "ldpc", "MIT", "BP and BP-OSD; opt-in flooding fork"),
...
def main() -> None:
    ...
    for name, (url, module, license_name, role) in SOURCES.items():
        ...
        for rel in PATHS[name]:
            path=directory/rel
            source_paths.extend([p for p in path.rglob('*') if p.is_file() and p.suffix in ('.py','.pyx','.hpp','.cpp')]
        ...
        for filename in sorted(untracked):
            ...
            if path.suffix not in ('.py','.hpp','.cpp','.md') or '__pycache__' in str(path):
                continue
        ...
        records[name] = {"url": url, "commit": git("rev-parse", "HEAD"),
                         ...
                         "flags": ('-O3 -std=c++2a; reference: -O3 -std=c++17 -fno-fast-math -ffp-contract=off'
