...
class SearchSession {
    ...
    void queue_node(uint64_t id,bool terminal) {
        ...
        const bool candidate_ok=!terminal&&id!=0;
        if(candidate_ok&&(node.construction==ConstructionStatus::Feasible||node.construction==ConstructionStatus::DepthLimited)) {
...
public:
    ...
    SearchSession(std::shared_ptr<const Model> model,Settings settings,bool reference=false):
        ...
        tree_.resize(2*leaves_);nodes_.reserve(4096);
    }
