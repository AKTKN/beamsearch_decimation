#ifndef QEC_FRONTIER_HPP
#define QEC_FRONTIER_HPP
#include "frontier_telemetry.hpp"
...
class Decoder {
    struct Candidate {
        uint64_t node=0,latest_update=0;ldpc::hybrid::Snapshot state;
    ...
    ldpc::hybrid::StatefulMinSumSession bp_;ldpc::hybrid::Osd0Bridge osd_;
    ...
    struct Guard {std::atomic_flag& f;explicit Guard(std::atomic_flag& x):f(x){if(f.test_and_set())throw std::logic_error("frontier decoder is non-reentrant");}~Guard(){f.clear();}};
    ...
    bool submit(const Bits& bits,SolutionSource source,std::optional<uint32_t> cycle,
                ...
                std::optional<uint64_t> disagreements) {
        ...
        event.prediction=model_->predict(bits);event.cost=model_->cost(bits);event.hint_disagreements=disagreements;
    ...
    uint64_t disagreements(const Bits& bits,uint64_t node_id) const {
    ...
    Reals fields(uint64_t node_id) const {
        Reals out=model_->graph->weights;
        if(settings_.hint_mode==HintMode::Soft) for(auto [i,b]:search_.node(node_id).order_pattern)
            out[size_t(i)]=ldpc::hybrid::finite((b?-1.0:1.0)*ldpc::hybrid::finite(out[size_t(i)]+settings_.margin));
        return out;
    }
    ...
    bool future_work(size_t next_cycle) const {
        for(size_t c=next_cycle;c<settings_.expansions.size();++c) {
            if(search_.frontier_size()&&((settings_.goal_test==GoalTest::OnPop)||
               (!search_.counts.generation_frozen&&settings_.expansions[c]))) return true;
            if(settings_.bp_enabled&&telemetry_.bp.iterations<settings_.max_total_iterations&&
               settings_.max_iterations_per_visit&&
               settings_.max_total_iterations-telemetry_.bp.iterations>=settings_.max_iterations_per_visit&&
               (!beam_.empty()||(search_.guidance_size()&&settings_.admissions[c]))) return true;
        }
        return false;
    }
...
public:
    Decoder(const Rows& rows,int n,const Reals& probabilities,const Rows& observables,Settings settings,bool reference=false):
        ...
        search_(model_,settings_,reference),bp_(model_->graph,settings_.clip,settings_.alpha),osd_(model_->graph) {settings_.validate();}
    Result decode(const Bits& syndrome) {
        ...
        try {
            if(!settings_.expansions.empty()) search_.reset(syndrome);
            std::optional<uint32_t> first_cycle;
            for(uint32_t cycle=0;cycle<settings_.expansions.size();++cycle) {
                ...
                row.beam_before=beam_.size();row.expansion_budget=settings_.expansions[cycle];
                row.pool_budget=settings_.max_iterations_per_visit;
                ...
                auto search_result=search_.advance(settings_.expansions[cycle],[this](){return cap();},
                ...
                auto proposed=search_.proposals(settings_.admissions[cycle]);telemetry_.bp.proposals+=proposed.size();row.proposals=proposed.size();
                ...
                row.available_budget=settings_.bp_enabled?settings_.max_total_iterations-telemetry_.bp.iterations:0;
                ...
                for(size_t turn=0;turn<ids.size();++turn) {
                    ...
                    const uint64_t remaining=settings_.max_total_iterations-telemetry_.bp.iterations;
                    const uint64_t quota=(settings_.bp_enabled&&settings_.max_iterations_per_visit<=remaining)
                        ?settings_.max_iterations_per_visit:0;
                    if(!quota) continue;if(cap()){cpu_stop=true;break;}
                    ...
                    if(!fresh&&settings_.inheritance==Inheritance::RetainedAncestor) {
                        ...
                        if(const Candidate* donor=nearest_donor(frozen,id)) {bp_.restore(donor->state,shot_token_);update.donor_kind="ancestor";
                        ...
                        else {bp_.reset(syndrome,shot_token_);update.donor_kind="cold_seed";++telemetry_.bp.cold_initializations;}
                    } else {bp_.reset(syndrome,shot_token_);update.donor_kind=fresh?"cold_seed":"cold_reset";
                        if(fresh)++telemetry_.bp.cold_initializations;else ++telemetry_.bp.cold_resets;}
                    bool valid=bp_.replace_fields(fields(id));++telemetry_.bp.hint_transitions;
                    update.residual_before=model_->graph->residual_weight(bp_.decision(),syndrome);
                    std::optional<uint64_t> event;
                    if(valid) {uint64_t d=disagreements(bp_.decision(),id);submit(bp_.decision(),SolutionSource::BPTransition,cycle,id,update.id,std::nullopt,d);
                    ...
                    if(!valid) {
                        ...
                        while(remaining&&!advanced.valid&&!advanced.cap_hit) {
                            auto part=bp_.advance(int(std::min<uint64_t>(remaining,uint64_t(INT_MAX))),[this](){return cap();});
                    ...
                    if(advanced.valid) {uint64_t d=disagreements(bp_.decision(),id);submit(bp_.decision(),SolutionSource::BPIteration,cycle,id,update.id,std::nullopt,d);
                    ...
                    update.disagreements=disagreements(candidate.state.decision,id);update.usable=true;update.valid=valid;update.solution_event=event;
    ...
    Result fallback_or_finish(const char* reason) {
        ...
        if(settings_.llr_policy==LlrPolicy::ChannelOnly||beam_.empty()) {
            llrs=model_->graph->weights;for(auto& x:llrs)x=std::min(x,settings_.clip);call.policy=settings_.llr_policy==LlrPolicy::ChannelOnly?"channel_only":"best_retained_guided";
        } else {
            ...
            if(settings_.llr_policy==LlrPolicy::BestRetainedRelease) {bp_.restore(best.state,shot_token_);bp_.replace_fields(model_->graph->weights);
                llrs=bp_.llrs();call.policy="best_retained_release";call.actual_source="retained_released";}
            else {llrs=best.state.llrs;call.policy="best_retained_guided";call.actual_source="retained_guided";}
        }
