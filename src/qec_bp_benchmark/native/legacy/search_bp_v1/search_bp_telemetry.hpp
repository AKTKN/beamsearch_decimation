#ifndef QEC_SEARCH_BP_TELEMETRY_HPP
#define QEC_SEARCH_BP_TELEMETRY_HPP
...
struct SolutionEvent {
    ...
    double cost=0;std::optional<uint64_t> fixed_assignment_violations;bool improved=false;uint64_t incumbent_after=0;
...
struct UpdateRecord {
    ...
    std::optional<uint64_t> donor_node,donor_update,residual_before,residual_after,violations,rank,solution_event;
...
struct OsdRecord {
    const char* trigger="cycle_budget";const char* policy="best_retained";
    const char* actual_source="channel";std::optional<uint64_t> node_id,update_id,solution_event;
...
struct BPSummary {
    ...
    uint64_t snapshot_copies=0,copied_bytes=0,hard_fixations=0,evictions=0,retired=0;
