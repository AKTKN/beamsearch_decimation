...
struct ColumnTable {
    py::dict columns;
    void append(const py::dict& row) {
        for(auto item:row) {
            if(!columns.contains(item.first)) columns[item.first]=py::list();
            py::cast<py::list>(columns[item.first]).append(item.second);
        }
    }
};
inline py::list columns_to_rows(const py::dict& columns) {
    py::list rows;if(py::len(columns)==0)return rows;
    auto first=*columns.begin();const py::ssize_t count=py::ssize_t(py::len(first.second));
    for(py::ssize_t i=0;i<count;++i) {py::dict row;
        for(auto item:columns)row[item.first]=py::reinterpret_borrow<py::sequence>(item.second)[i];
        rows.append(std::move(row));}
    return rows;
}
inline py::dict result_summary(const Result& result,const Telemetry& telemetry) {
    py::dict d;d["algorithm_version"]="SEARCH-BP-1.0";d["status"]=result.status;
    ...
    search["prefix_cpu_cap_hit"]=s.prefix_cpu_cap_hit;search["rho_min_final"]=c.rho_min;
    ...
    bp["snapshot_copies"]=b.snapshot_copies;bp["copied_bytes"]=b.copied_bytes;bp["hard_fixations"]=b.hard_fixations;
...
inline py::dict telemetry_columns(const Telemetry& telemetry) {
    py::dict out;ColumnTable cycles,patterns,updates,memberships,solutions,osd,nodes,phases;
    ...
    for(const auto& r:telemetry.updates){py::dict x;x["update_id"]=r.id;x["cycle_index"]=r.cycle;x["node_id"]=r.node_id;
        ...
        x["residual_after"]=optional_u64(r.residual_after);x["fixed_assignment_violations_after"]=optional_u64(r.violations);
    ...
    for(const auto& r:telemetry.solutions){py::dict x;x["solution_event_id"]=r.id;x["cycle_index"]=optional_u32(r.cycle);x["source"]=source_name(r.source);
        ...
        x["correction"]=r.correction;x["prediction"]=r.prediction;x["physical_cost"]=r.cost;x["fixed_assignment_violations"]=optional_u64(r.fixed_assignment_violations);
    ...
    out["cycles"]=cycles.columns;out["patterns"]=patterns.columns;out["bp_updates"]=updates.columns;out["bp_beam_membership"]=memberships.columns;
    ...
    out["solution_events"]=solutions.columns;out["osd_calls"]=osd.columns;out["phase_timings"]=phases.columns;out["search_nodes"]=nodes.columns;return out;
}
inline py::dict telemetry_rows(const Telemetry& telemetry) {
    auto columns=telemetry_columns(telemetry);py::dict out;
    for(auto item:columns)out[item.first]=columns_to_rows(py::cast<py::dict>(item.second));
    return out;
}
inline void bind_search_bp(py::module_& module) {
    py::class_<Settings>(module,"SearchBPSettings").def(py::init<>())
        .def_readwrite("max_depth",&Settings::max_depth).def_readwrite("max_cycles",&Settings::max_cycles)
        .def_readwrite("det_beam",&Settings::det_beam).def_readwrite("prefix_cpu_budget_ns",&Settings::prefix_cpu_budget_ns)
        .def_readwrite("expansions_per_cycle",&Settings::expansions_per_cycle)
        .def_readwrite("beam_width",&Settings::beam_width).def_readwrite("max_iteration",&Settings::max_iteration)
        .def_readwrite("post_solution_cycles",&Settings::post_solution_cycles).def_readwrite("alpha",&Settings::alpha)
        .def_readwrite("bp_enabled",&Settings::bp_enabled)
        ...
        .def("set_llr_policy",[](Settings& s,const std::string& x){s.llr_policy=x=="channel_only"?LlrPolicy::ChannelOnly:LlrPolicy::BestRetained;});
    py::class_<Result>(module,"SearchBPResult").def_readonly("valid",&Result::valid).def_readonly("correction",&Result::correction)
        .def_readonly("prediction",&Result::prediction).def_readonly("cost",&Result::cost).def_readonly("status",&Result::status);
    py::class_<Decoder>(module,"SearchBPDecoder")
        ...
        .def("summary",[](Decoder& decoder,const Result& result){return result_summary(result,decoder.telemetry_view());})
        .def("export_telemetry",[](Decoder& decoder){return telemetry_rows(decoder.telemetry_view());})
        .def("export_telemetry_columns",[](Decoder& decoder){return telemetry_columns(decoder.telemetry_view());});
}
}
#endif
